...
- [ ] Record ADR 021 with the processor identity, thread-affine lifetime,
      processor-group, error, restoration, and cross-target contracts.
- [ ] Implement the typed index and non-Send RAII guard without allocation or
      a compatibility surface; re-export through the Hermes facade.
- [ ] Cover exact bind/current/restore and invalid-no-mutation behavior on
      Windows; cover explicit unsupported behavior and cross-target builds.
- [ ] Run host/AArch64 warning-denied gates, doctests, Rustdoc, SemVer, lock,
      independent review, and Apollo adoption before merge.
...
- [x] Add one same-binary, value-gated four-versus-eight accumulator instrument
      without changing established workloads or timed regions.
- [x] Run two bounded cache-resident f32/f64 sum/dot measurements; the candidate
      failed the repeated-win gate before exact-codegen qualification.
- [x] Retain only a repeatable material correction with generic differential and
      boundary coverage; otherwise delete the candidate and record rejection.
- [x] Run the exact host Clippy, 523-test Nextest, doctest, Rustdoc, formatting,
      diff, and standalone-lock gates; cross-target and independent source
      review are unaffected because the rejected candidate leaves no code.
- [ ] Publish and merge the compact measurement record, then continue with the
      next measured gap.
...
- [x] Publish and merge PR #95; carry its stale AArch64 F16 lane-oracle
      correction in the next exact-diff delivery.

## HS-MASKED-TAIL-PARTIAL-LOAD-2026-08-27 [minor]

- [x] Reconcile full-width masked contracts, every dispatch-tail caller, scalar
      and backend implementations, governing ADRs, and current conformance.
- [x] Record the exact short-tail baseline, allocation/stack mechanism, and
      emitted code before changing production behavior.
- [x] Add one partial masked seam whose inactive lanes are never dereferenced;
...
- [x] Record ADR 019 and assign each SpMV format implementation to one leaf
      module before retaining the dense-tail correction.
- [x] Record exact scalar baselines for representative narrow-band regimes and
