...
- **Verified defect.** `docs/adr/README.md` opens with
  `Generated by scripts/adr-index.py — do not hand-edit`, naming
  `python scripts/adr-index.py generate` and `... check`. **No such script
  exists in hermes** — `scripts/` contains only `lockfile.py`. The header is
  a convention copied from a sibling repo whose generator was never vendored.
- **Consequences, both live now:** nothing validates the index, so an ADR can
  be added, renamed, or have its status changed with the index silently
  stale; and contributors are instructed *not* to hand-edit the one artifact
  that is now hand-editable. The hermes NumaBinding work (#120) hit exactly
  this — it had to reason about whether editing was permitted, concluded
  hand-editing was the only option, and happened not to need an edit.
- **The script exists upstream.** apollo and kwavers both carry
  `scripts/adr-index.py`; leto, themis, mnemosyne, moirai, and hephaestus do
  not. So this is not only a hermes gap — it is one generator, two copies,
  and five repos claiming or wanting the convention without it.
- **Decision required before implementing** (do not vendor a third copy on
  autopilot):
  - **(a) Vendor into hermes.** Smallest diff, unblocks the check in CI now,
    makes it three divergent copies of one generator.
  - **(b) Consolidate first.** Establish one home — an xtask, or a shared
    scripts source the stack members pull — then adopt everywhere claiming
    the convention. Correct by consolidation_discipline, larger, and blocks
    hermes on a stack-wide decision.
  - **(c) Delete the false header.** Honest immediately, abandons the
    validation the header advertises. Acceptable only if the convention is
    genuinely not wanted here.
  Recommend **(a) now, (b) as a filed stack item** — the index is unvalidated
  today, and a third copy is a known, bounded debt with an existing consumer,
