...
//!
//! Exposes a transparent wrapper `Vector<T, Arch>` over the architecture-native
//! vector type `Arch::Vector`, with standard operator overloading, constructors,
...
use hermes_simd_core::scalar::{Scalar, CastFrom};
use hermes_simd_core::mask::BitMask;

/// A monomorphized vector register type wrapping the architecture-native raw register.
///
/// `Vector<T, Arch>` is `#[repr(transparent)]` and has the exact same layout and size as
/// `Arch::Vector`. It delegates hot-path arithmetic (addition, subtraction, multiplication)
/// directly to `SimdKernel` traits for zero runtime overhead.
#[repr(transparent)]
    ...
    T: Scalar,
{
    ...
    pub fn from_view_chunk<Align, Mode, Ref>(
        view: &hermes_simd_core::view::SimdView<'_, T, Arch, Align, Mode, Ref>,
        chunk_idx: usize,
    ) -> Self
    where
    ...
    pub fn store_to_view_chunk<'a, Align, Mode>(
        self,
        view: &mut hermes_simd_core::view::SimdView<'a, T, Arch, Align, Mode, &'a mut [T]>,
        chunk_idx: usize,
    )
    where
        Align: hermes_simd_core::align::Alignment,
...
///
/// `Mask<T, Arch>` is a zero-cost transparent wrapper over the native mask representation `Arch::Mask`.
/// It provides bitwise operations, reductions (`any`, `all`, `none`), and conversions
/// to/from the portable `BitMask<N>`.
#[repr(transparent)]
