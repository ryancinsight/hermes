...
impl BackendKernel<f64> for Avx512 {
    ...
    unsafe fn fmsub(a: Self::Vector, b: Self::Vector, c: Self::Vector) -> Self::Vector {
        ...
        Avx512F64Vec(_mm512_fmsub_pd(a.0, b.0, c.0))
    }
    ...
    unsafe fn transpose_square(tile: &mut [Self::Vector]) {
        ...
        // The canonical three-stage 8x8 network, 24 shuffles: `unpack` weaves
        // within each 128-bit block, then two rounds of `shuffle_f64x2` move
        // whole blocks. The default would round-trip the tile through the
        // stack instead.
        let mut t = [_mm512_setzero_pd(); 8];
        for k in 0..4 {
            t[2 * k] = _mm512_unpacklo_pd(tile[2 * k].0, tile[2 * k + 1].0);
            t[2 * k + 1] = _mm512_unpackhi_pd(tile[2 * k].0, tile[2 * k + 1].0);
        }
        let mut u = [_mm512_setzero_pd(); 8];
        for k in 0..2 {
            for o in 0..2 {
                u[4 * k + o] = _mm512_shuffle_f64x2::<0x88>(t[4 * k + o], t[4 * k + o + 2]);
                u[4 * k + o + 2] = _mm512_shuffle_f64x2::<0xdd>(t[4 * k + o], t[4 * k + o + 2]);
            }
        }
        for o in 0..4 {
            tile[o] = Avx512F64Vec(_mm512_shuffle_f64x2::<0x88>(u[o], u[o + 4]));
            tile[o + 4] = Avx512F64Vec(_mm512_shuffle_f64x2::<0xdd>(u[o], u[o + 4]));
        }
    }
    ...
    unsafe fn deinterleave_pairs(a: Self::Vector, b: Self::Vector) -> (Self::Vector, Self::Vector) {
        // Alternating 64-bit lane pairs of `a || b`.
        let even_idx = _mm512_setr_epi64(0, 1, 4, 5, 8, 9, 12, 13);
        let odd_idx = _mm512_setr_epi64(2, 3, 6, 7, 10, 11, 14, 15);
        (
            Avx512F64Vec(_mm512_permutex2var_pd(a.0, even_idx, b.0)),
            Avx512F64Vec(_mm512_permutex2var_pd(a.0, odd_idx, b.0)),
        )
    ...
    unsafe fn interleave_halves(a: Self::Vector, b: Self::Vector) -> (Self::Vector, Self::Vector) {
        // Each half is two 128-bit blocks: `shuffle_f64x2` takes blocks 0-1 of
    ...
    unsafe fn concat_shift_pairs_at(a: Self::Vector, b: Self::Vector, k: usize) -> Self::Vector {
        ...
        unsafe {
            match k {
                1 => <Self as BackendKernel<f64>>::concat_shift_pairs::<1>(a, b),
                2 => <Self as BackendKernel<f64>>::concat_shift_pairs::<2>(a, b),
                _ => <Self as BackendKernel<f64>>::concat_shift_pairs::<3>(a, b),
            }
        }
    }
    ...
    unsafe fn bitand(a: Self::Vector, b: Self::Vector) -> Self::Vector {
        Avx512F64Vec(_mm512_castsi512_pd(_mm512_and_si512(
    ...
    unsafe fn bitor(a: Self::Vector, b: Self::Vector) -> Self::Vector {
        Avx512F64Vec(_mm512_castsi512_pd(_mm512_or_si512(
            _mm512_castpd_si512(a.0),
            _mm512_castpd_si512(b.0),
        )))
    }
    ...
    unsafe fn bitxor(a: Self::Vector, b: Self::Vector) -> Self::Vector {
        Avx512F64Vec(_mm512_castsi512_pd(_mm512_xor_si512(
            _mm512_castpd_si512(a.0),
            _mm512_castpd_si512(b.0),
        )))
    }
    ...
    unsafe fn abs(a: Self::Vector) -> Self::Vector {
        let sign_mask = _mm512_castpd_si512(_mm512_set1_pd(-0.0));
        Avx512F64Vec(_mm512_castsi512_pd(_mm512_andnot_si512(
            sign_mask,
            _mm512_castpd_si512(a.0),
        )))
    }
